...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($_GET['action'] == 'approve') {
        ...
        $insertQuery = "INSERT INTO barang_rusak (assetno, returnfaileddate, returnreceiveby) VALUES (
            '".$data['assetno']."', 
            '".$data['returnfaileddate']."', 
            '".$data['returnreceiveby']."')";
        
        if (mysqli_query($conn, $insertQuery)) {
            // Set notification for successful approval
            $_SESSION['notification'] = 'Data berhasil disetujui.';
        } else {
            $_SESSION['notification'] = 'Error inserting into barang_rusak: ' . mysqli_error($conn);
        }
...
<body>
    <div class="container mt-5">
        ...
        <a href="rusak.php" class="btn btn-primary mb-3">Back</a>
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($requests)) : ?>
                    <tr>
                        ...
                        <td><?php echo ($row['returnreceiveby']); ?></td>
                        <td><?php echo ($row['returnfaileddate']); ?></td>
                        <td>
